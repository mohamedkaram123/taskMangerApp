...
import { error_state, isAuth, post } from '../../helper';
import Urls, { routes, urls } from '../../urls';
  ...
  const [loginInput, setloginInput] = useState({
    ...
    "password": "",
      ...
      const fetch_login = async () => {
        try {
          const res = await post(urls.login_url,loginInput)
          if(res.statusMsg == "success"){
            encryptCookieStorage(res.data,"user")
            navigate(routes.home_route);
            setloadSearch(false)
          }else{
    ...
    <div className="px-6 h-full text-gray-800">
      <div className="flex xl:justify-center lg:justify-between justify-center items-center flex-wrap h-full g-6">
          <div className="grow-0 shrink-1 md:shrink-0 basis-auto xl:w-6/12 lg:w-6/12 md:w-9/12 mb-12 md:mb-0">
            <img
              src={ process.env.REACT_APP_ASSEST_URL + "img/Task-bro.png"}
              className="w-full"
              alt="Sample image"
            />
          </div>
          <div className="xl:ml-20 xl:w-5/12 lg:w-5/12 md:w-8/12 mb-12 md:mb-0">
              <form>
                  <div className="mb-10 text-center">
                    <h1  className="font-bold text-3xl text-indigo-600" style={{ fontWeight:800 }}>Task Manger Admin</h1>
                  </div>
